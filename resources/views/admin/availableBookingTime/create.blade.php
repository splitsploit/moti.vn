...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.availableBookingTime.title_singular') }}
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('admin.availableBookingTime.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('week_days') ? 'is-invalid' : '' }}" name="week_days[]" id="week_days" multiple required>
                        @foreach ($weekDays as $id => $week_days)
                            <option value="{{ $id }}" {{ in_array($id, old('week_days', [])) ? 'selected' : '' }}>{{ $week_days }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group {{ $errors->has('end') ? 'has-error' : '' }}">
                    ...
                    <input type="text" id="end" name="end" class="form-control available-booking-time-timepicker"
                        value="{{ old('end', isset($availableBookingTime) ? $availableBookingTime->end : '') }}" required>
                    @if ($errors->has('end'))
                        <em class="invalid-feedback">
                            {{ $errors->first('end') }}
                        </em>
                ...
                <div class="form-group">
                    <label class="required" for="maximum">{{ trans('cruds.availableBookingTime.fields.maximum') }}</label>
